...
using UnityEngine;
using CharacterController2D = CometaPrototypes.CharacterController2D.CharacterController2D;
using CharacterStates = CometaPrototypes.CharacterController2D.CharacterStates;
...
namespace Prototypes.SlimeParty
{
    public class SlimePartyCharacter : MonoBehaviour
    {
        ...
        {
            _horizontalMovement = _playerInput.Movement.x;

            if (_playerInput.JumpButton.State.CurrentState == MMInput.ButtonStates.ButtonDown)
            {
                JumpStart();
            }
            ...
            {
                if ((_playerInput.JumpButton.TimeSinceLastButtonDown < InputBufferDuration) && (Time.time - _lastJumpAt > InputBufferDuration))
                {
                    NumberOfJumpsLeft = NumberOfJumps;
                    _doubleJumping = false;
                    _inputBuffer = true;
                    _jumpButtonPressed = (_playerInput.JumpButton.State.CurrentState == MMInput.ButtonStates.ButtonPressed);
                    _jumpButtonPressTime = Time.time;
                    _jumpButtonReleased = (_playerInput.JumpButton.State.CurrentState == MMInput.ButtonStates.ButtonPressed);
                    _lastInputBufferJumpAt = Time.time;
                    JumpStart();
                }
            }

            if (_playerInput.JumpButton.State.CurrentState == MMInput.ButtonStates.ButtonUp)
            {
                JumpStop();
            }
        }
